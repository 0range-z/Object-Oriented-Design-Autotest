...
from HW6_generator import generate
from HW6_judger import judge

MODE = '.jar'  # .java: compile and run    .class .jar: run
FILE_NAME_LIST = ['Assassin', 'Berserker', 'Caster', 'Lancer', 'Me', 'Saber', 'Rider']
...
def check(data):
    ...
    for i in FILE_LIST:
        ...
        fout.close()
        judge_result = judge(data, out_lines, i[2], time_dict, WrongLog, single_time)
        if not judge_result:
            dt = ""
            for line in data:
                if line == "END\n":
...
if __name__ == "__main__":
    ...
    print("修改FILE_NAME_LIST为你的.jar名，HW6_generator里的TIME_MAX也可以改")
    ...
    while True:
        ...
        if a == "q":
            ...
            NUM_ELEVATOR = int(input("请输入电梯个数\n"))
